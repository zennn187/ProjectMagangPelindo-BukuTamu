        ...
        <main class="p-4 sm:p-6 2xl:p-8 transition-all xl:ml-64 min-h-screen">
            @hasSection('page_title')
                <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="hidden md:flex items-center gap-2 rounded-full bg-bank-light/70 px-4 py-2 w-56 lg:w-72">
                            <svg class="h-4 w-4 text-bank-gray shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 10.5a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0z"/></svg>
                            <input type="text" placeholder="Cari kunjungan, pegawai..." class="bg-transparent text-sm text-bank-navy placeholder-bank-gray focus:outline-none w-full">
                        </div>
            ...
            @endif
            @if(session('success'))
                <div class="mb-5 rounded-2xl bg-white border-l-4 border-bank-green text-bank-navy px-4 py-3 text-sm shadow-sm">{{ session('success') }}</div>
            @endif
            @if($errors->any())
                <div class="mb-5 rounded-2xl bg-white border-l-4 border-bank-red text-bank-navy px-4 py-3 text-sm shadow-sm">{{ $errors->first() }}</div>
            @endif
            @yield('content')
        </main>
